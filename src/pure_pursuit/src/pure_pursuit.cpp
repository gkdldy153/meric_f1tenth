...
PurePursuit::PurePursuit() : Node("pure_pursuit_node") {
    ...
    this->declare_parameter("waypoints_path", "/home/meric/f1tenth_ws/src/waypoint_generator/src/waypoints_odom1.csv");
...
void PurePursuit::publish_message(double steering_angle) {
    ...
    const double lim = to_radians(steering_limit);
    if (steering_angle >  lim) steering_angle =  lim;
    if (steering_angle < -lim) steering_angle = -lim;

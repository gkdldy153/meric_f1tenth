...
class StanleyAvoidance(Node):
    def __init__(self):
        ...
        self.declare_parameter("waypoints_path", "/home/meric/f1tenth_ws/src/waypoint_generator/src/waypoints_odom.csv")
        self.declare_parameter("waypoints_path_2nd", "/home/meric/f1tenth_ws/src/waypoint_generator/src/waypoints_odom.csv")
        ...
        self.declare_parameter("min_lookahead_speed", 2.0)
        ...
        self.twist_pub = self.create_publisher(Twist, '/stanley_vel', 10)
...
class WaypointUtils:
    def __init__(
        ...
        filepath="/home/meric/f1tenth_ws/src/waypoint_generator/src/waypoints_odom.csv",
        min_lookahead=0.5,
        max_lookahead=3.0,
        min_lookahead_speed=3.0,
        max_lookahead_speed=6.0,
        filepath_2nd="/home/meric/f1tenth_ws/src/waypoint_generator/src/waypoints_odom.csv",
        lane_number=0,
    ):
    ...
    def load_and_interpolate_waypoints(self, file_path, interpolation_distance=0.05):
        try:
            ...
            if raw_data.shape[1] < 3:
                self.node.get_logger().warn("Velocity column not found, filling with 2.0 m/s")
                velocities = np.full((raw_data.shape[0],), 2.0)
            else:
        ...
        if interpolation_distance != 0 and interpolation_distance is not None:
            ...
            # 방어 코드
            if np.isnan(total_distance) or total_distance <= 0:
                raise ValueError(f"[WaypointUtils] Invalid total distance: {total_distance}")

            segments = max(int(total_distance / interpolation_distance), 2)
            self.node.get_logger().info(f"Interpolating with {segments} segments (total {total_distance:.2f} m)")

            # 선형 보간
            distance = np.cumsum(distances)
            distance = np.insert(distance, 0, 0) / distance[-1]  # 정규화
